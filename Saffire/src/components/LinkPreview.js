...
class LinkPreview extends Component {
  ...
  render () {
    ...
    for (let key in this.props.likedBy) {
      ...
      <div className="link-container">
        ...
        <div className="link-buttons">
          ...
          {(!this.props.hasBeenAdded && this.props.connect) &&
            <div
                ...
                label={`Like ${this.props.likes}`}
                onClick={() => this.props.newLike(this.props.eventKey, this.props.itinKey)}
                disabled={((likedByArray.indexOf(this.props.user.email)) > -1) || !this.props.connect}>
                JOIN EVENT
            </div>
          }

          {(!this.props.hasBeenAdded && this.props.isOwner && this.props.connect) &&
            <div
                className="linkpreview-hover"
                label="Add To Itinerary"
                disabled={!this.props.connect}
                onClick={() => this.props.confirmEvent(this.props.eventKey, this.props.itinKey)}>
                ADD TO ITINERARY
            </div>}
        ...
        {this.state.showComments && 
          <div className="ideaboard-comments">
            {(this.state.comments && !this.state.newComments.length) &&
                Object.keys(this.state.comments).map(key => (
              ...
              <section className="google-details-main">
                ...
                <div className="google-details-info">
                    {this.props.googleDetails.openingHours &&
                        this.props.googleDetails.openingHours.open_now ? 'OPEN NOW' : ''
                    }

                  <div className="google-details-openingHours">
                      {this.props.googleDetails.openingHours &&
                        this.props.googleDetails.openingHours.weekday_text ? this.props.googleDetails.openingHours.weekday_text.map(day => {return <span>{day}</span>}) : ''
                      }
                  </div>
                  ...
                  <div className="google-details-reviews">
                      {this.props.googleDetails.reviews &&
                        this.props.googleDetails.reviews.map(review => {
                          return (
                              <div className="google-individual-review">
